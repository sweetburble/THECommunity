...
Future<void> requestPermission() async {
  // 연락처에 접근하는 권한을 요청한다, 변수에는 권한을 허락/거부 했는지 저장된다, 다양한 거부 타입이 있다
  final contactPermissionStatus = await Permission.contacts.request();

  // 알림 권한을 요청한다 -> flutter_local_notifications 패키지가 푸시 메시지를 보내기 위해 사용한다
  final notificationsPermissionStatus = await LocalNotifications.requestNotificationsPermission() ?? false;

  // 권한 상태가 거부/영구 거부일 때는 기기의 앱 설정 화면으로 이동하여, 사용자가 직접 허용하도록 한다
...
void main() async {
  ...
  await remoteConfig.setConfigSettings(RemoteConfigSettings(
    fetchTimeout: const Duration(minutes: 1), // 가져오기 타임아웃
    minimumFetchInterval: const Duration(hours: 1), // 최소 가져오기 간격 (프로덕션에서는 더 길게)
  ));
