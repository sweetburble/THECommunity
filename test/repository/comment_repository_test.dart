...
void main() {
  ...
  group('getCommentList method', () {
    ...
    test('Test Case 1.1: Feed has multiple comments', () async {
      ...
      await createTestFeed(
          feedId: feedWithCommentsId, writerUid: 'feedWriterUser');
      ...
      await createTestComment(
          feedId: feedWithCommentsId,
          commentId: 'comment1',
          writerUid: user1Id,
          commentText: 'Comment by UserOne',
          createdAt: timestamp1);
      await createTestComment(
          feedId: feedWithCommentsId,
          commentId: 'comment2',
          writerUid: user2Id,
          commentText: 'Newer Comment by UserTwo',
          createdAt: timestamp2);
      await createTestComment(
          feedId: feedWithCommentsId,
          commentId: 'comment3',
          writerUid: user1Id,
          commentText: 'Older Comment by UserOne',
          createdAt: timestamp3);

      // --- ACT ---
      final List<CommentModel> comments =
          await commentRepository.getCommentList(feedId: feedWithCommentsId);

      // --- ASSERT ---
      expect(comments.length, 3, reason: "Should fetch all 3 comments");

      // Verify order (descending by createAt)
      expect(comments[0].commentId, 'comment2',
          reason: "First comment should be the newest");
      expect(comments[1].commentId, 'comment1',
          reason: "Second comment should be middle one");
      expect(comments[2].commentId, 'comment3',
          reason: "Third comment should be the oldest");

      // Verify writer population
      final commentFromUser1 = comments.firstWhere(
          (c) => c.writer.uid == user1Id && c.commentId == 'comment1');
      final commentFromUser2 = comments.firstWhere(
          (c) => c.writer.uid == user2Id && c.commentId == 'comment2');

      expect(commentFromUser1.writer.nickname, 'UserOne',
          reason: "Comment writer UserOne's nickname should be populated");
      expect(commentFromUser2.writer.nickname, 'UserTwo',
          reason: "Comment writer UserTwo's nickname should be populated");
    ...
    test('Test Case 1.2: Feed has no comments', () async {
      ...
      await createTestFeed(
          feedId: feedWithoutCommentsId, writerUid: 'someUser');
      // No comments added to this feed.

      // --- ACT ---
      final List<CommentModel> comments =
          await commentRepository.getCommentList(feedId: feedWithoutCommentsId);

      // --- ASSERT ---
      expect(comments.isEmpty, isTrue,
          reason: "Returned list should be empty for a feed with no comments");
    });
  });
  ...
  group('uploadComment method', () {
    ...
    test('Test Case 2.1: Successfully upload a comment', () async {
      ...
      await createTestFeed(
          feedId: targetFeedId, writerUid: 'feedOwner', initialCommentCount: 0);

      // --- ACT ---
      final CommentModel uploadedComment =
          await commentRepository.uploadComment(
        feedId: targetFeedId,
      ...
      expect(commentsSnapshot.docs.length, 1,
          reason: "One comment document should be created");
      ...
      expect(commentDataFromFirestore['commentId'], commentDocFromFirestore.id,
          reason: "commentId field should match document ID");
      expect(commentDataFromFirestore['writer'], userDocRef(commenterUserId),
          reason: "Comment writer reference should be correct");
      expect(commentDataFromFirestore['comment'], commentText,
          reason: "Comment text should be correct");
      expect(commentDataFromFirestore['createAt'], isA<Timestamp>(),
          reason: "createAt should be a Timestamp");

      // 3. Verify commentCount increment on the feed document
      final feedDocAfterComment = await feedDocRef(targetFeedId).get();
      expect(feedDocAfterComment.data()?['commentCount'], 1,
          reason: "Feed's commentCount should be incremented");

      // 4. Verify the returned CommentModel
      expect(uploadedComment.commentId, commentDocFromFirestore.id,
          reason: "Returned model's commentId is correct");
      expect(uploadedComment.writer.uid, commenterUserId,
          reason: "Returned model's writer UID is correct");
      expect(uploadedComment.comment, commentText,
          reason: "Returned model's comment text is correct");
      expect(uploadedComment.createAt, isNotNull,
          reason: "Returned model's createAt should be populated");

      // Verify populated writer in returned CommentModel
      expect(uploadedComment.writer, isA<UserModel>(),
          reason: "Returned model's writer should be a UserModel");
      expect(uploadedComment.writer.uid, commenterUserId,
          reason: "Returned model's writer UID is correct");
      expect(uploadedComment.writer.nickname, 'Commenter',
          reason: "Returned model's writer nickname is correct");
    });
  });
}
